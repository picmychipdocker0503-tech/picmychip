...
test.describe('Frontend', () => {
  ...
  async function checkout(
    ...
    const { cardNumber, expiryDate, cvc } = paymentDetails

    // Unlike Razorpay/Stripe's in-page checkout, PayU's standard integration is a full-page
    // redirect — the browser navigates away to PayU's own hosted domain (test.payu.in in test
    // mode), so this interacts with PayU's real top-level page, not an iframe on our site.
    // Selectors below follow PayU's commonly documented hosted-checkout field names — confirm
    // against a live render (with real test-mode PAYU_MERCHANT_KEY/PAYU_MERCHANT_SALT) before
    // relying on this in CI; PayU's page markup isn't something this environment can verify live.
    await page.waitForURL(/test\.payu\.in|secure\.payu\.in/)

    const cardOption = page.getByText(/credit\s*\/?\s*debit card/i)
    if (await cardOption.isVisible().catch(() => false)) {
      await cardOption.click()
    }

    await page.locator('input[name="ccnum"]').fill(cardNumber)
    await page.locator('input[name="ccExpMon"], select[name="ccExpMon"]').fill(expiryDate.slice(0, 2))
    await page.locator('input[name="ccvv"]').fill(cvc)

    const payButton = page.getByRole('button', { name: /pay now|make payment/i })
