...
export const ReviewsListView: React.FC<ListViewServerProps> = async ({ BeforeList, searchParams, user }) => {
  ...
  const result = await payload.find({
    ...
    sort,
    user,
    where: search ? { comment: { contains: search } } : undefined,
  })

  const initialData: ListApiResponse<Review> = {
    docs: result.docs,
  ...
  return (
    <div className="gutter--left gutter--right" style={{ paddingBlock: '2rem' }}>
      <h1 className="mb-1 text-2xl font-bold">Reviews</h1>
      <p className="text-base-content/70 mb-6 text-sm">Manage product reviews.</p>
      {BeforeList}
      <ReviewsTable initialData={initialData} />
    </div>
  )
}
