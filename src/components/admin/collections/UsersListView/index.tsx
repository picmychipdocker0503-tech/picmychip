...
export const UsersListView: React.FC<ListViewServerProps> = async ({ BeforeList, searchParams, user }) => {
  ...
  const result = await payload.find({
    ...
    sort,
    user,
    where: search ? { or: [{ name: { contains: search } }, { email: { contains: search } }] } : undefined,
  })

  const initialData: ListApiResponse<User> = {
    docs: result.docs,
  ...
  return (
    <div className="gutter--left gutter--right" style={{ paddingBlock: '2rem' }}>
      ...
      <p className="text-base-content/70 mb-6 text-sm">Customer and admin accounts.</p>
      {BeforeList}
