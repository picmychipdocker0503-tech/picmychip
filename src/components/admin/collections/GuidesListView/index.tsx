...
export const GuidesListView: React.FC<ListViewServerProps> = async ({ BeforeList, searchParams, user }) => {
  ...
  const initialData: ListApiResponse<Guide> = {
    docs: result.docs,
  ...
  return (
    <div className="gutter--left gutter--right" style={{ paddingBlock: '2rem' }}>
      <h1 className="mb-1 text-2xl font-bold">Guides</h1>
      <p className="text-base-content/70 mb-6 text-sm">Manage build guides and component comparisons.</p>
      {BeforeList}
      <GuidesTable initialData={initialData} />
    </div>
  )
}
