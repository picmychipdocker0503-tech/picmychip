...
import { DatasheetsTable } from './DatasheetsTable'
...
export const DatasheetsListView: React.FC<ListViewServerProps> = async ({ BeforeList, searchParams, user }) => {
  ...
  const initialData: ListApiResponse<Datasheet> = {
    docs: result.docs,
  ...
  return (
    <div className="gutter--left gutter--right" style={{ paddingBlock: '2rem' }}>
      <h1 className="mb-1 text-2xl font-bold">Datasheets</h1>
      <p className="text-base-content/70 mb-6 text-sm">Manage datasheet and spec-sheet files.</p>
      {BeforeList}
      <DatasheetsTable initialData={initialData} />
    </div>
  )
}
