...
export const StockAlertsListView: React.FC<ListViewServerProps> = async ({ BeforeList, searchParams, user }) => {
  ...
  const initialData: ListApiResponse<StockAlert> = {
    docs: result.docs,
  ...
  return (
    <div className="gutter--left gutter--right" style={{ paddingBlock: '2rem' }}>
      <h1 className="mb-1 text-2xl font-bold">Stock Alerts</h1>
      <p className="text-base-content/70 mb-6 text-sm">Back-in-stock notification requests from customers.</p>
      {BeforeList}
      <StockAlertsTable initialData={initialData} />
    </div>
  )
}
