...
export const ProductsListView: React.FC<ListViewServerProps> = async ({ BeforeList, searchParams, user }) => {
  ...
  const result = await payload.find({
    ...
    sort,
    user,
    where: search ? { or: [{ title: { contains: search } }, { sku: { contains: search } }] } : undefined,
  })

  const initialData: ListApiResponse<Product> = {
    docs: result.docs,
  ...
  return (
    <div className="gutter--left gutter--right" style={{ paddingBlock: '2rem' }}>
      ...
      <p className="text-base-content/70 mb-6 text-sm">Manage the product catalog.</p>
      {BeforeList}
